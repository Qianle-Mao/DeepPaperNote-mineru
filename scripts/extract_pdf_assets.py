...
from mineru_v4 import image_files
...
def _mineru_full_markdown(extract_dir: Path) -> str:
...
def _markdown_image_refs(markdown: str) -> list[dict]:
    refs: list[dict] = []
...
def _caption_label_pattern(label: str) -> re.Pattern | None:
    normalized = normalize_whitespace(label)
    match = re.match(r"^(fig(?:ure)?|table)\.?\s*([A-Za-z]?\d+[a-z]?)$", normalized, flags=re.IGNORECASE)
    if not match:
        return None
    prefix = "table" if match.group(1).lower().startswith("table") else "fig(?:ure)?"
    number = re.escape(match.group(2))
    return re.compile(rf"\b{prefix}\.?\s*{number}\b", flags=re.IGNORECASE)


def _best_caption_label_match(markdown: str, pattern: re.Pattern) -> re.Match | None:
    matches = list(pattern.finditer(markdown))
    if not matches:
        return None

    def score(match: re.Match) -> tuple[int, int]:
        following = markdown[match.end() : match.end() + 16]
        preceding = markdown[max(0, match.start() - 2) : match.start()]
        captionish = 0
        if following.startswith((".", ":", "：", "。")):
            captionish += 10
        if preceding.endswith("\n\n") or preceding.endswith("\n"):
            captionish += 2
        return captionish, match.start()

    return sorted(matches, key=score, reverse=True)[0]


def _mineru_markdown_figure_matches(record: dict, extract_dir: Path) -> list[dict]:
    markdown = _mineru_full_markdown(extract_dir)
    if not markdown:
        return []
    image_refs = _markdown_image_refs(markdown)
    if not image_refs:
        return []
    captions = record.get("captions", {}) if isinstance(record.get("captions"), dict) else {}
    figure_captions = captions.get("figures", []) if isinstance(captions.get("figures"), list) else []
    matches: list[dict] = []
    used_filenames: set[str] = set()
    for caption in figure_captions:
        if not isinstance(caption, dict):
            continue
        label = normalize_whitespace(str(caption.get("id", "")))
        pattern = _caption_label_pattern(label)
        if pattern is None:
            continue
        label_match = _best_caption_label_match(markdown, pattern)
        if not label_match:
            continue
        caption_pos = label_match.start()
        before = [
            (caption_pos - int(ref["pos"]), ref)
            for ref in image_refs
            if int(ref["pos"]) < caption_pos and caption_pos - int(ref["pos"]) <= 2500
        ]
        after = [
            (int(ref["pos"]) - caption_pos, ref)
            for ref in image_refs
            if int(ref["pos"]) > caption_pos and int(ref["pos"]) - caption_pos <= 4500
        ]
        picked = None
        if before:
            picked = sorted(before, key=lambda item: item[0])[0][1]
        elif after:
            picked = sorted(after, key=lambda item: item[0])[0][1]
        if not picked:
            continue
        filename = str(picked.get("filename", ""))
        ...
        matches.append(
            {
                "label": label,
                "filename": filename,
                "page_number": int(caption.get("page", 1) or 1),
                "caption_distance_chars": abs(caption_pos - int(picked["pos"])),
            }
        )
    return matches
...
def _page_number_from_name(path: Path) -> int:
    match = re.search(r"(?:page|pg|p)[_-]?0*(\d+)", path.stem, flags=re.IGNORECASE)
    if match:
...
def _label_from_name(path: Path) -> str:
    ...
    fig = re.search(r"\bfig(?:ure)?\s*\.?\s*(\d+[a-z]?)\b", text, flags=re.IGNORECASE)
    if fig:
        return f"Fig {fig.group(1)}"
    table = re.search(r"\btable\s*\.?\s*(\d+[a-z]?)\b", text, flags=re.IGNORECASE)
    if table:
        return f"Table {table.group(1)}"
    return ""


def mineru_assets_payload(record: dict, args: argparse.Namespace) -> dict | None:
    mineru = record.get("mineru", {}) if isinstance(record.get("mineru"), dict) else {}
    extract_dir_value = str(mineru.get("extract_dir", "")).strip()
    if not extract_dir_value:
        return None
    extract_dir = Path(extract_dir_value).expanduser()
    ...
    copied_by_source_name: dict[str, dict] = {}
    for index, source in enumerate(image_files(extract_dir), start=1):
        dest_name = source.name
        if (images_dir / dest_name).exists():
            dest_name = f"mineru_{index:03d}_{source.name}"
        dest = images_dir / dest_name
        shutil.copy2(source, dest)
        page_number = _page_number_from_name(source)
        asset = {
            ...
            "extraction_level": "mineru_zip",
        }
        copied_images.append(asset)
        copied_by_source_name[source.name] = asset
        label = _label_from_name(source)
        if label:
            figure_assets.append(
                {
                    **asset,
                    ...
                    "quality_signals": {
                        ...
                        "reason": "mineru_extracted_image_asset",
                    },
                }
            )

    for match in _mineru_markdown_figure_matches(record, extract_dir):
        asset = copied_by_source_name.get(str(match.get("filename", "")))
        if not asset:
            continue
        label = normalize_whitespace(str(match.get("label", "")))
        if any(normalize_whitespace(str(item.get("label", ""))).lower() == label.lower() for item in figure_assets):
            continue
        figure_assets.append(
            {
                **asset,
                "page_number": int(match.get("page_number", asset.get("page_number", 1)) or 1),
                "label": label,
                "quality_signals": {
                    "visual_quality_status": "usable",
                    "reason": "mineru_markdown_caption_nearby_image",
                    "caption_distance_chars": int(match.get("caption_distance_chars", 0) or 0),
                },
            }
        )

    source_pages = record.get("pages", []) if isinstance(record.get("pages"), list) else []
    page_records = []
    pages_with_images: dict[int, int] = {}
    for asset in copied_images:
        pages_with_images[asset["page_number"]] = pages_with_images.get(asset["page_number"], 0) + 1
    for page in source_pages:
        if not isinstance(page, dict):
            continue
        page_number = int(page.get("page", 1) or 1)
        page_records.append(
            {
                "page_number": page_number,
                "searchable_text_chars": int(page.get("char_count", 0) or 0),
                "text_extraction_method": "mineru_v4_ocr" if mineru.get("is_ocr") else "mineru_v4",
                "ocr_used": bool(mineru.get("is_ocr")),
                "image_count": pages_with_images.get(page_number, 0),
                "figure_count": len([asset for asset in figure_assets if int(asset.get("page_number", 0) or 0) == page_number]),
                "page_text": "",
                "text_preview": "",
            }
        )
    if not page_records:
        page_records = [
            {
                "page_number": 1,
                "searchable_text_chars": 0,
                "text_extraction_method": "mineru_v4",
                "ocr_used": bool(mineru.get("is_ocr")),
    ...
    payload = {
        ...
        "pdf_path": str(record.get("pdf", {}).get("path", "")) if isinstance(record.get("pdf"), dict) else "",
        "asset_root": str(asset_root),
        "images_dir": str(images_dir),
        "page_assets": page_records,
        "image_assets": copied_images,
        "figure_assets": figure_assets,
        "asset_coverage": {
            "total_pages": len(page_records),
            "asset_max_pages": args.max_pages,
            "asset_pages_scanned": len(page_records),
            "truncated_due_to_asset_page_limit": False,
            "source": "mineru_v4_zip",
        },
        "ocr_available": True,
        "source_kind": "mineru_v4_assets",
    }
    return payload
...
def main() -> None:
    ...
    mineru_payload = mineru_assets_payload(record, args)
    if mineru_payload is not None:
        emit(mineru_payload, args.output)
        return

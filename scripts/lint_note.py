...
INNOVATION_SECTION = "11. 创新点判断"
KEY_RESULTS_SECTION = "9. 实验设计深度分析"
REFERENCES_SECTION = "1. 论文基本信息"
LIMITATIONS_SECTION = "12. 局限性与潜在问题"
METHOD_SECTION = "6. 方法框架详解"
DEEP_ANALYSIS_SECTION = "10. 与代表性工作的关系"
FLOW_SECTION = "8. 模型结构图或算法流程复现"
...
def front_matter_order_warnings(text: str) -> list[str]:
    warnings: list[str] = []
    required_order = ["## 1. 论文基本信息", "## 2. 一句话总结", "## 3. 导师导读"]
    positions = []
...
def mixed_language_issues(text: str) -> list[dict[str, object]]:
    ...
    for idx, line in enumerate(lines, start=1):
        ...
        if section_name in {REFERENCES_SECTION, "核心信息", "引用"}:
            continue
...
def core_info_structure_issues(text: str) -> list[dict[str, object]]:
    heading, body = section_body_any(text, (REFERENCES_SECTION, "核心信息"))
    if not body:
        return []
    ...
    last_known_index = -1
    base_line = _line_number_from_offset(text, text.find(f"## {heading}"))
...
def section_body_any(text: str, headings: tuple[str, ...]) -> tuple[str, str]:
...
def inspect_substantive_content(text: str) -> list[dict[str, object]]:
    ...
    for section in REQUIRED_SECTIONS:
        ...
        if section not in {KEY_RESULTS_SECTION, REFERENCES_SECTION, "关键结果", "引用"} and is_honest_missing_declaration(content):
            issues.append(issue(section, "section_honest_missing_not_allowed", "error", content))

    innovation_heading, innovation = section_body_any(text, (INNOVATION_SECTION, "创新点"))
    ...
    if not innovation_units:
        issues.append(issue(innovation_heading, "innovation_empty_shell", "error", innovation_content))
    elif len(innovation_units) < 2:
        issues.append(issue(innovation_heading, "innovation_too_few_specific_points", "warning", innovation_content))

    key_results_heading, key_results = section_body_any(text, (KEY_RESULTS_SECTION, "关键结果"))
    ...
    references_heading, references = section_body_any(text, (REFERENCES_SECTION, "引用"))
    ...
    limitations_heading, limitations = section_body_any(text, (LIMITATIONS_SECTION, "局限"))
    ...
    for section in (METHOD_SECTION, DEEP_ANALYSIS_SECTION):
        body = section_body(text, section)
        content = normalized_section_content(body)
...
def method_section_requires_mechanism_flow(text: str) -> bool:
    _heading, body = section_body_any(text, (METHOD_SECTION, "方法主线"))
    if not body:
        return False
    lower = body.lower()
...
def mechanism_flow_warnings(text: str) -> list[str]:
    ...
    if not method_section_requires_mechanism_flow(text):
        return warnings
    if f"## {FLOW_SECTION}" in text:
        body = section_body(text, FLOW_SECTION)
    elif "### 机制流程" in text:

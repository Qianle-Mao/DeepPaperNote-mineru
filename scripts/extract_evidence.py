...
def load_raw_sections_from_manifest(record: dict) -> list[dict]:
    raw_path = str(record.get("raw_sections_path", "")).strip()
    if not raw_path:
        return []
...
def section_map_from_raw_sections(sections: list[dict]) -> dict[str, str]:
    section_map: dict[str, list[str]] = {}
    for section in sections:
        kind = normalize_whitespace(str(section.get("kind", ""))).lower() or "section"
        title = normalize_whitespace(str(section.get("title", "")))
        text = str(section.get("text", "")).strip()
        if not text:
            continue
        normalized_kind = kind
        if kind == "preamble":
            normalized_kind = "abstract" if "abstract" in text.lower() else "preamble"
        elif kind in {"related-work", "background"}:
            normalized_kind = "introduction"
        elif kind in {"models", "model"}:
            normalized_kind = "method"
        section_map.setdefault(normalized_kind, []).append(f"{title}\n{text}" if title else text)
    return {key: "\n\n".join(parts) for key, parts in section_map.items()}


def full_text_from_raw_sections(sections: list[dict]) -> str:
    return "\n\n".join(
...
def main() -> None:
    ...
    source_manifest_mode = bool(record.get("raw_sections_path"))
    ...
    has_pdf = (not source_manifest_mode) and pdf_path is not None and pdf_path.is_file()
    if source_manifest_mode:
        raw_sections = load_raw_sections_from_manifest(record)
        if not raw_sections:
            extraction_failures.append("source_manifest_raw_sections_missing")
        section_map = section_map_from_raw_sections(raw_sections)
        full_text = full_text_from_raw_sections(raw_sections)
        pdf_coverage = record.get("coverage", {}) if isinstance(record.get("coverage"), dict) else {}
        appendix_index = record.get("appendix_index", {}) if isinstance(record.get("appendix_index"), dict) else {}
        appendix_evidence = build_appendix_evidence([], appendix_index)
    else:
        pdf_coverage = (

---
name: deeppapernote-mineru
description: MinerU-backed fork of DeepPaperNote for generating high-quality deep-reading notes for a single paper. Use when the user wants the DeepPaperNote evidence grounding, figure/table decision, lint, and Obsidian-save workflow, with MinerU v4 precise API OCR parsing and the customized data-mining/AI note format.
---

# DeepPaperNote-mineru

This skill is a MinerU-backed customization fork of DeepPaperNote.

Default stance:
- preserve the original source manifest, figure/table decision, grounding lint, final lint, and Obsidian save workflow
- use MinerU v4 precise API as the default document parsing backend instead of PyMuPDF
- apply local customizations in this fork rather than editing the upstream DeepPaperNote skill
- keep changes small and testable, especially when changing scripts or lint gates

## Custom Parsing Backend

DeepPaperNote-mineru uses MinerU v4 precise API for document parsing.

Required configuration:
- set `MINERU_API_TOKEN` or `DEEPPAPERNOTE_MINERU_API_TOKEN` in the environment
- never hard-code the MinerU token in `SKILL.md`, scripts, notes, or logs

Default MinerU options:
- API mode: v4 precise API
- local files: request upload URL with `/api/v4/file-urls/batch`, upload the file, poll `/api/v4/extract-results/batch/{batch_id}`, then download `full_zip_url`
- model version: `vlm`
- OCR: enabled by default
- formula recognition: enabled by default
- table recognition: enabled by default
- language: `ch` by default

Environment overrides:
- `MINERU_MODEL_VERSION` or `DEEPPAPERNOTE_MINERU_MODEL_VERSION`
- `MINERU_IS_OCR` or `DEEPPAPERNOTE_MINERU_IS_OCR`
- `MINERU_ENABLE_FORMULA` or `DEEPPAPERNOTE_MINERU_ENABLE_FORMULA`
- `MINERU_ENABLE_TABLE` or `DEEPPAPERNOTE_MINERU_ENABLE_TABLE`
- `MINERU_LANGUAGE` or `DEEPPAPERNOTE_MINERU_LANGUAGE`
- `MINERU_PAGE_RANGES` or `DEEPPAPERNOTE_MINERU_PAGE_RANGES`
- `MINERU_TIMEOUT_SECONDS` or `DEEPPAPERNOTE_MINERU_TIMEOUT_SECONDS`
- `MINERU_POLL_INTERVAL_SECONDS` or `DEEPPAPERNOTE_MINERU_POLL_INTERVAL_SECONDS`

Fallback policy:
- do not silently fall back to PyMuPDF
- PyMuPDF fallback is allowed only when `DEEPPAPERNOTE_CUSTOM_ALLOW_PYMUPDF_FALLBACK=1`
- if MinerU fails and fallback is not explicitly enabled, stop and report the blocked parsing stage
...
- required structure: for data-mining and AI papers, use the 精读版 sections in `references/data-mining-ai-note-format.md` rather than the legacy DeepPaperNote section set
- abstract handling: preserve the original abstract's key information where useful, but do not force a standalone abstract-translation section unless the user asks for the legacy format
- mechanism depth: method, framework, and system papers should explain the core model or algorithm flow in `## 8. 模型结构图或算法流程复现`, normally with 3 to 4 clear input-operation-output steps
...
- consult `references/data-mining-ai-note-format.md` before drafting the final note; its 精读版 format is the default output contract for data-mining and AI papers
...
- For data-mining and AI papers, the final note must follow `references/data-mining-ai-note-format.md` 精读版 by default: `# 论文精读笔记` plus 16 numbered `##` sections from `1. 论文基本信息` through `16. 总评`.
- `## 1. 论文基本信息` must be a fixed metadata-and-judgment block. Use the fields from `references/data-mining-ai-note-format.md`; do not add free prose in this section.
- Do not include the old fixed-section template (`核心信息`, `原文摘要翻译`, `创新点`, `研究问题`, `方法主线`, etc.) unless the user explicitly asks to use the legacy format.
- The note should preserve the original abstract's key information inside the new sections when useful, but the default output no longer requires a standalone `原文摘要翻译` section.
- `## 11. 创新点判断` is the required innovation section; it must distinguish claimed contributions from contributions that actually stand up to reviewer scrutiny.
...
- use the 精读版 top-level sections from `references/data-mining-ai-note-format.md` for data-mining and AI papers, then use `contracts_by_paper_type[note_plan.paper_type].section_semantics` and the user's research-direction needs to decide the internal emphasis of each section
